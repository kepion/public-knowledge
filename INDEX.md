...
- [planning-app-model-vs-application-rules](planning-app-model-vs-application-rules.md) — the model-scoped save is the only one that binds a rule to a model; the application-scoped one writes a different object with no binding, and the cached read-back hides it — verify metadata writes against the store, not the API
- Something else
